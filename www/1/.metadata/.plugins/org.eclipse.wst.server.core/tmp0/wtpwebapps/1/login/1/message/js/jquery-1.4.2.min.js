...
c.ready()}}function Qa(a,b) {
	b.src?c.ajax( {
	url:b.src,async:false,dataType:"script"
}
):c.globalEval(b.text||b.textContent||b.innerHTML||"");
	b.parentNode&&b.parentNode.removeChild(b)}function X(a,b,d,f,e,j) {
	var i=a.length;
	if(typeof b==="object") {
...
this.context=s;
	this.selector=a;
	return this}else if(!b&&/^\w+$/.test(a)) {
	this.selector=a;
	this.context=s;
...
,removeAttr:function(a) {
	...
	for(var j=" "+e.className+" ",i=e.className,o=0,k=b.length;
	o<k;
...
else e.className=""}return this},toggleClass:function(a,b) {
...
}return f}if(Ba.test(b.type)&&!c.support.checkOn)return b.getAttribute("value")===null?"on":b.value;
...
u.push(j);
	c.event.global[k]=true}a=null}}},global: {
...
if(!a.which&&(a.charCode||a.charCode===0?a.charCode:a.keyCode))a.which=a.charCode||a.keyCode;
	...
	return a},guid:1E8,proxy:c.proxy,special: {
	ready: {
...
,live: {
	...
	b&&c.event.remove(this,a.origType,oa)}},beforeunload: {
...
)}else return false},teardown:function() {
...
,teardown:function() {
	...
	ea=c.event.special.change.filters}s.addEventListener&&c.each( {
...
return h}function b(g,h,l,m,q,p) {
	q=0;
	for(var v=m.length;
	q<v;
	q++) {
...
t=t[g]}m[q]=y}}}function d(g,h,l,m,q,p) {
	q=0;
	for(var v=m.length;
	q<v;
	q++) {
...
q("previousSibling",h,m,g,p,l)}},find: {
	ID:function(g,h,l) {
...
g[0]=e++;
	return g},ATTR:function(g,h,l,m,q,p) {
	h=g[1].replace(/\\/g,"");
...
else k.error("Syntax error,unrecognized expression:"+q)},CHILD:function(g,h) {
	var l=h[1],m=g;
	switch(l) {
...
g=g.nodeIndex-q;
	return l===0?g===0:g%l===0&&g/l>=0}},ID:function(g,h) {
...
g=n.relative[g]?g+"*":g;
	...
	else b=c.filter(b,f)}return c.grep(a,function(e) {
...
for(;
	...
	delete j[i]}}f=f.parentNode}}return d}var k=c.expr.match.POS.test(a)?c(a,b||this.context):null;
...
return f},nth:function(a,b,d) {
	b=b||1;
	for(var f=0;
	a;a=a[d])if(a.nodeType===1&&++f===b)break;
	return a
}
...
f.parentNode&&f.parentNode.removeChild(f)}return this},empty:function() {
	for(var a=0,b;
	(b=this[a])!=null;
...
)}else return this.pushStack(c(c.isFunction(a)?a():a),"replaceWith",a)},detach:function(a) {
...
o.length&&c.each(o,Qa)}return this}});
	c.fragments= {
	};
	c.each( {
...
return f.filter&&f.filter.indexOf("opacity=")>=0?parseFloat(Oa.exec(f.filter)[1])/100+"":""}if(ha.test(b))b=Pa;
	b=b.replace(ia,ja);
	if(e)f[b]=d;
	...
	return Math.max(0,Math.round(e))}return c.curCSS(a,b,d)},curCSS:function(a,b,d) {
	var f,e=a.style;
...
r||x.setRequestHeader("X-Requested-With","XMLHttpRequest");
	x.setRequestHeader("Accept",e.dataType&&e.accepts[e.dataType]?e.accepts[e.dataType]+",*/*":e.accepts._default)}catch(ga) {
...
e.async||g();
	return x}},handleError:function(a,b,d,f) {
...
c.data(this[a],"olddisplay",f)}}a=0;
	for(b=this.length;
	a<b;
...
this.startTime=J();
	...
	var e=this;
	f.elem=this.elem;
	if(f()&&c.timers.push(f)&&!W)W=setInterval(c.fx.tick,13)},show:function() {
...
f=d;
	d=b.offsetParent}if(c.offset.subtractsBorderForOverflowNotVisible&&j.overflow!=="visible") {
	k+=parseFloat(j.borderTopWidth)||0;
	n+=parseFloat(j.borderLeftWidth)||0
}
f=j}if(f.position==="relative"||f.position==="static") {
	k+=o.offsetTop;
	n+=o.offsetLeft
}
...
return {
	...
	a.removeChild(b);
	c.offset.initialize=c.noop},bodyOffset:function(a) {
